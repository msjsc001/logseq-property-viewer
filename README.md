# Property Query - Logseq 属性查询与统计工具 v2.4.0
...
> **v2.4.0 可用性升级**：新增值统计、页首属性区解析、大小写敏感查询开关与模式化语法说明，修复 `ai-提示词` 这类页首属性漏检问题，并过滤 `--:: --` 这类伪属性污染。
> **v2.3.0 架构升级**：重建查询引擎与块解析器，修复精确匹配失真、嵌套块串值、固定端口误判、非标准目录无法监听等核心问题，并补齐数据质量诊断、分类清理、块定位与测试基建。
...
### 🔍 高级属性查询
- **多种匹配模式**：
  - `key:value` — 精确匹配（默认大小写不敏感，按完整值比对）
  - `key~value` — 模糊匹配（默认大小写不敏感，按子串比对）
  - `has:key` — 存在性匹配
  - `text` — 全文搜索（匹配页面名、块内容、属性键和值）
- **大小写敏感开关**：搜索框右侧提供 `Aa` 开关，可让 `has / 精确 / 模糊 / 全文` 全部切换到大小写敏感模式
- **页首属性支持**：支持解析首个 `- block` 之前的 Logseq 页面属性区，像 `ai-提示词:: ...` 这类属性不会再漏检
- **伪属性过滤**：仅识别真正的 `key:: value` 双冒号属性，自动过滤 `--:: --`、`--2:: --` 等分隔噪音
...
- **块级定位**：除页面跳转外，可辅助定位结果所在块的上下文位置

### 📊 数据统计与分析
- 一键扫描知识库，统计所有属性键
- **值统计**：支持全局值分布统计，并可下钻查看某个值分别出现在什么键下
- **刷新按钮**：手动刷新统计数据
...
- **统一目录策略**：标准 Logseq 图谱优先扫描 `pages/` 与 `journals/`，非标准目录支持递归扫描与递归监听
...
- **分类清理**：支持按缓存、日志、界面偏好、查询历史、图谱路径分别清理
- **3秒确认机制**：防止误操作的延迟确认
...
| `/api/cache/build`   | POST     | 全量扫描并重建指定图谱缓存               |
| `/api/search`        | POST     | 高级查询引擎（支持精确/模糊/存在/全文 + 大小写开关） |
| `/api/stats`         | GET      | 全局属性键统计                           |
| `/api/stats/global-values` | GET | 全局值统计                               |
| `/api/stats/value-keys` | GET  | 指定值在不同属性键中的分布               |
| `/api/stats/diagnostics` | GET | 数据质量诊断结果下发                     |
| `/api/preferences`   | GET      | 查询历史、语言、列配置、大小写偏好等用户偏好 |
